...
public class kXMLElement {
	 ...
	 * @see nanoxml.kXMLElement#kXMLElement(java.util.Hashtable)
	 */
	public kXMLElement(Hashtable conversionTable, boolean skipLeadingWhitespace) {
	 ...
	 * conversions: <CODE>&amp;amp; &amp;lt; &amp;gt;
	 *         &amp;apos; &amp;quot;</CODE> (depending on
	 * <I>fillBasicConversionTable</I>)</DD>
	 * <DT>Skip whitespace in contents</DT>
	 * <DD><I>skipLeadingWhitespace</I></DD>
	 * <DT>Ignore Case</DT>
	 ...
	 */
	public kXMLElement(Hashtable conversionTable,
			boolean skipLeadingWhitespace, boolean ignoreCase) {
	...
	protected kXMLElement(Hashtable conversionTable,
			boolean skipLeadingWhitespace, boolean fillBasicConversionTable,
			boolean ignoreCase) {
	 ...
	 */
	public int getIntProperty(String key, Hashtable valueSet,
			String defaultValue) {
    ...
    */
	 ...
	 * <I>defaultValue</I> is returned.
	 */
	public Object getProperty(String key, Hashtable valueSet,
			String defaultValue) {
		if (ignoreCase) {
			key = key.toUpperCase();
		}
	 ...
	 * <I>defaultValue</I> is returned.
	 */
	public String getStringProperty(String key, Hashtable valueSet,
			String defaultValue) {
		if (ignoreCase) {
			key = key.toUpperCase();
		}
	 ...
	 * be an integer. If the property doesn't exist, the value corresponding to
	 * <I>defaultValue</I> is returned.
	 */
	public int getSpecialIntProperty(String key, Hashtable valueSet,
			String defaultValue) {
		if (ignoreCase) {
			key = key.toUpperCase();
		}
	 ...
	 */
	public void parseString(String string) throws kXMLParseException {
	 ...
	 *                if an error occured while parsing the string
	 */
	public int parseString(String string, int offset) throws kXMLParseException {
	 ...
	 */
	public int parseString(String string, int offset, int end,
			int startingLineNr) throws kXMLParseException {
	 ...
	 *                if an error occured while parsing the array
	 */
	private int parseCharArray(char[] input, int offset, int end,
			int[] currentLineNr) throws kXMLParseException {
		...
		offset = scanContent(input, offset, end, contentOffset, contentSize,
				currentLineNr);
	 ...
	 *                if an error occured while parsing the array
	 */
	private void processContents(char[] input, int contentOffset,
			int contentSize, int contentLineNr) throws kXMLParseException {
		int[] lineNr = new int[1];
	 ...
	 *                if an error occured while parsing the array
	 */
	protected void scanChildren(char[] input, int contentOffset,
			int contentSize, int contentLineNr) throws kXMLParseException {
	...
	protected kXMLElement createAnotherElement() {
		return new kXMLElement(conversionTable, skipLeadingWhitespace, false,
				ignoreCase);
	}
	 ...
	 *                if an error occured while parsing the array
	 */
	private int scanContent(char[] input, int offset, int end,
			int[] contentOffset, int[] contentSize, int[] lineNr)
			throws kXMLParseException {
		if (input[offset] == '/') {
			contentSize[0] = 0;
			...
			if (input[offset] == '<') {
				...
				if (ok) {
					...
					if (ok) {
						...
						try {
							offset = skipWhitespace(input, offset, end
									+ tag.length + 2, lineNr);
						} catch (kXMLParseException e) {
							// ignore
						}
		...
		while (index < s.length()) {
			...
			if ((index2 <= index3) && (index2 <= index4)) {
				...
				if (key.charAt(0) == '#') {
					...
					result.append(getProperty(conversionTable, key, "&" + key
							+ ';'));
				}
	 ...
	 */
	protected void writeEncoded(kXMLPrintWriter out, String str) {
	 ...
	 */
	private kXMLParseException invalidValue(String key, String value, int lineNr) {
	 ...
	 */
	private kXMLParseException syntaxError(String context, int lineNr) {
	 ...
	 */
	private kXMLParseException expectedInput(String charSet, int lineNr) {
	 ...
	 */
	private kXMLParseException valueMissingForAttribute(String key, int lineNr) {
