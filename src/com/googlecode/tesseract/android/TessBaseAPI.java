...
public class TessBaseAPI {
     ...
     */
    /**
     *tessbaseapi可同时被做个线程引用
     *datapath tessdata所在目录的父目录b，且必须以/结尾。
     *要引用多种语言 则language 表达式为   eng+chi-sim  ,~表示用来取反，eng+~chi-sim,表示强制使用eng
   	 *引用的语言的数量只与机器的内存大小相关，引用的数量越多效率越低
     *调用完init()以后要调用end(),将参数值恢复到默认值，这样的习惯比较好
     * 
     * @param datapath
     * @param language
     * @return
    ...
    /**
     * 
     * 初始化Tesseract 引擎，并制定语言模型
     * 参数同上
     * @param datapath
     * @param language
     * @param ocrEngineMode 语言模型
     * @return
     */
     ...
     */
    /**
     * 1.关闭  tesseract 引擎，释放所有的内存空间
     * 2.对TessBaseAPI的参数重新初始化，相当于重新构造一个TessBaseAPI
     * 3.当这个函数被调用以后，所有的提取文字的代码都会终止。(包含多个线程同时执行的情况)，只有重新调用init(),
     * 才能重新执行提取文字的工作
     * 
     */
    public void end() {
    ...
    /**
     * 
     * 设置需要识别为图片，直到调用GetUTF8Chars之前，该图片的内容必须是不变的
     * @param bmp   需要识别的位图
     */
